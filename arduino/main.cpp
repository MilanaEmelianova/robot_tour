...
#define MM_PER_REV              260     // Set to the number of mm per wheel revolution (Hence : Diameter * Pi)

#define ENCODER_COUNTS_90_DEG   550     // Set to the number of encoder pulses to make a 90 degree turn
...
class MotionLogic
{
  ...
  public:
    ...
    int isStopped() {
      if (running == 0 && speedActual <= 10) return 1;
      return 0;
    }
    ...
    void startMove(int pos, int spd) {
      ...
      if (debugPrint) { Serial.print("time (us),encoder"); }
      if (debugPrint) { Serial.print(",speed"); }
      if (debugPrint) { Serial.print(",pos error"); }
      if (debugPrint) { Serial.print(",speed error"); }
      if (debugPrint) { Serial.print(",I"); }
      if (debugPrint) { Serial.print(",P"); }
      if (debugPrint) { Serial.println(",pwm"); }
    ...
    void updateMotion(long usecElapsed) {
      ...
      float speed;
      //if (debugPrint) { Serial.print(timeRunning); }
      if (timeRunning < timeAtSpeed) {
        speed = (float) timeRunning / 1000000.0 * (float) accelRate;
        speedProfile = (int) speed;
        // Serial.print(" - Accel speedProfile = "); Serial.println(speedProfile);
      } else if (timeRunning < timeDecel) {
        speedProfile = speedTarget;
        // Serial.print(" - At Speed speedProfile = "); Serial.println(speedProfile);
      } else {
        ...
        // Serial.print(" - Decel speedProfile = "); Serial.println(speedProfile);
      }
      ...
      if (debugPrint) { Serial.print(","); Serial.print(countEncoder); }
      if (debugPrint) { Serial.print(","); Serial.print(speedProfile); }
      if (debugPrint) { Serial.print(","); Serial.print(perror); }
      if (debugPrint) { Serial.print(","); Serial.print(serror); }
      if (debugPrint) { Serial.print(","); Serial.print(pwmLoopI); }
      if (debugPrint) { Serial.print(","); Serial.print(pwmLoopP); }
      if (debugPrint) { Serial.print(","); Serial.println(outputPWM); }
